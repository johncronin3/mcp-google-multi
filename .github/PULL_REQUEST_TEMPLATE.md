...
- [ ] If this touches a module published via the package `exports` map (see `package.json` — `identity`, `compose`, `registry`, `oauth-as`, `accounts`, `token-store`, `http-transport`, `client`, `config-file`, `fs-atomic`, `master-key`, `mcp-token`, `boot-gates`, `tenant-purge`, `doctor`, `http-config`, `write-control`, `scope-catalog`, `auth`), the exported signatures are unchanged — or the change is called out in the PR description and `MIGRATION-v6.md`
